...
NORM_STRING = /^(?<type>DIN EN|DIN|EN|ISO|VDA|OEMSPEC|OEMSTD)\s+(?<num>[0-9A-Za-z]{1,16})(?:(?<sep>[.\-])(?<sub>[0-9A-Za-z]+))?\s*$/
```

- `type` : kontrollierte Präfixliste (erweiterbar).
- `num`  : alphanumerische Kernnummer (1 bis 16 Zeichen).
...
def parse_norm_string(raw: str) -> dict:
    ...
    m = re.match(r"^([0-9A-Za-z]{1,16})(?![0-9A-Za-z])(?:([.\-])([0-9A-Za-z]+))?(.*)$", rest)
    if not m:
