...
The `TestExecution` contains tester, start time, a specimen reference,
measurement system and individual assessor grades.

### Specimen

The topic-level `Specimen` references the concrete tested `ComponentInstance`
through `ComponentInstanceID`. The test execution references the specimen
through `SpecimenID`.
...
- `TestingProject.Topics[].Specimens[].ComponentInstanceID`
- `TestingProject.Topics[].Executions[].SpecimenID`
...
- `ComponentInstance.MaterialSourceID` references the concrete source used for
  the produced part.
- The tested part is referenced by `TestSeries.Specimens[].ComponentInstanceID`,
  and the execution selects that specimen through `TestExecution.SpecimenID`.
...
This draft is intentionally not schema-conformant because it contains the
proposed `MaterialSources`, `MaterialSource.Identifiers` and
`ComponentInstance.MaterialSourceID` structures and proposed controlled values
...
- Tested-part reference: `TestSeries.Specimens[].ComponentInstanceID`
- Execution-to-specimen reference: `TestExecution.SpecimenID`
