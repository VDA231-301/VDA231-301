...
v3.0.0. Validation against this schema version should be performed
...
It identifies the tester, start date, specimen reference, measurement system
and individual assessor grades.

### Specimen

The topic-level `Specimen` represents the tested sample. The test execution
references it through `SpecimenID`.
...
- `TestingProject.Topics[].Specimens[]`
...
`Specimens[].ComponentInstanceID`; the execution selects that specimen through
`SpecimenID`.
...
- The concrete test execution is maintained in
  `TestingProject.Topics[].Executions[]`.
- The topic-level specimen links to the tested part through
  `TestingProject.Topics[].Specimens[].ComponentInstanceID`, and the execution
  references the specimen through `SpecimenID`.
...
- Tested specimen:
  `TestingProject.Topics[].Specimens[]`
- Execution-to-specimen reference:
  `TestingProject.Topics[].Executions[].SpecimenID`
