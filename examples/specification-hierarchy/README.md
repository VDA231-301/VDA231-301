...
The modelling pattern is defined in the entity relationship model, supported
by ADR 0005 and represented by `Specification.ReferencedSpecifications` in the
...
This example is intended to align with generic schema v3.0.0, in which a
`Specification` can reference other `Specification` objects through
`Specification.ReferencedSpecifications`. The self-reference and its reference
semantics are supported by ADR 0005.

Validation against the referenced schema version should be performed before
productive use.
